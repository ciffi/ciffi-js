...
export default (function() {
  class Title extends React.Component {
    render() {
      return <h1>Config for {this.props.text}</h1>
    }
  }

  class Toggler extends React.Component {
    render() {
      let _text = !this.props.isActive
  ...
  class List extends React.Component {
    render() {
      ...
      Object.entries(this.props.items).map(item => {
        if (typeof item[0] !== 'object' && typeof item[1] !== 'object') {
          ...
          Object.entries(item[1]).map(childItem => {
            if (
              typeof childItem[0] !== 'object' &&
              typeof childItem[1] !== 'object'
            ) {
              _list.push(
                <Item
                  key={childItem[0]}
                  name={`${item[0]} - ${childItem[0]}`}
                  value={childItem[1]}
                />
              )
            }
      ...
      return <ul style={{ opacity: _opacity }}>{_list}</ul>
    }
  }
    ...
    render() {
      return (
        <div>
          ...
          <hr />
          <List items={this.props.items} isVisible={this.state.isVisible} />
          <Toggler
            defaultText={`Hide`}
            activeText={`Show`}
